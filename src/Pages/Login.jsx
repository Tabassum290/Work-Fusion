...
const Login = () => {
    const {user,setUser,LoginUser,loading,setLoading,googleProvider} = useContext(AuthContext);
    ...
    e.preventDefault();
    const uservalue = capchaRef.current.value;
...
<DotLottieReact
	...
	<form onSubmit={handleLogin} className="space-y-6">
		...
		<div className="space-y-1 text-sm">
			...
			<input ref={capchaRef} type="text" name="capcha" placeholder="capcha" className="w-full px-4 py-3 rounded-md
             dark:border-gray-300 dark:bg-gray-50 dark:text-gray-800 focus:dark:border-violet-600" />
             <button onClick={handleValidateCapcha} className='btn btn-xs btn-outline w-full'>Validate</button>
		</div>
		<button disabled={disabled}  className="btn bg-[#856432b3] w-full p-3 text-center text-white rounded-sm dark:text-white dark:bg-violet-600">Sign in</button>
	</form>
